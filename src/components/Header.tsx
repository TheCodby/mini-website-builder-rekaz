...
export const Header = memo<HeaderProps>(
  ({
    ...
    const [importModal, setImportModal] = useState<{
      ...
      message: string;
      warnings?: string[];
    }>({ show: false, type: "success", message: "", warnings: [] });
      ...
      try {
        ...
        if (result.success) {
          ...
          setImportModal({
            ...
            message:
              result.warnings && result.warnings.length > 0
                ? "Website imported successfully with some warnings!"
          ...
          setImportModal({
            show: true,
            type: "error",
            message: result.error || "Import failed due to an unexpected error",
          });
        }
      } catch (error) {
        ...
        <AnimatePresence>
          {importModal.show && (
            <motion.div
              ...
              onClick={() =>
                setImportModal({ show: false, type: "success", message: "" })
              }
                ...
                <div className="text-center">
                  ...
                  <button
                    ...
                    className={`w-full px-6 py-3 rounded-xl font-semibold text-white transition-all duration-200 ${
                      importModal.type === "success"
                        ? "bg-primary-500 hover:bg-primary-600 focus:ring-4 focus:ring-primary-200"
                        : "bg-red-500 hover:bg-red-600 focus:ring-4 focus:ring-red-200"
                    }`}
                  >
                    {importModal.type === "success" ? "Great!" : "Try Again"}
                  </button>
                </div>
              </motion.div>
            </motion.div>
